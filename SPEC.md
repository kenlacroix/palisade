...
- `evidence` and any raw payloads encrypted at rest (per-org key envelope) — **implemented** (migration 0005). A per-org data key (DEK) is wrapped by a master key-encryption key (`PALISADE_EVIDENCE_KEK`, base64 32-byte AES-256) and stored in `org_encryption_key`; the plaintext DEK is never persisted. With a KEK set, ingest seals evidence into `finding.evidence_enc` (AES-256-GCM, `nonce||ciphertext`) and empties `finding.evidence`; all read paths go through `encryption.open_evidence`. With no KEK configured (dev/SQLite) evidence stays plaintext JSON and the helpers are pass-throughs. `org_encryption_key` is intentionally **not** under RLS (workers read it without the per-request org GUC; lookups filter by `org_id` in code).
